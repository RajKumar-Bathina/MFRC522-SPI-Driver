...
class MFRC522 {
   public:
    ...
    enum PCD_Register {
        ...
        TestPinValueReg = 0x34 << 1,  // defines the values for D1 to D7 when it is
                                 // used as an I/O bus
    ...
    MFRC522(spi_device_handle_t spiHandle);
    ...
    byte AuthenticateBlock(byte *uid, byte uidLen, uint32_t blockNumber, byte keyNumber, byte *keyData);
    void ReadBlock(byte currentBlock);
    bool IsTrailerBlock (uint32_t uiBlock);
    bool IsFirstBlock (uint32_t uiBlock);
