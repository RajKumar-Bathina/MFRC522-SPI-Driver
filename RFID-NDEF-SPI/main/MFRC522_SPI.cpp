...
/**
 * Constructor.
 * Creates a MFRC522 object.
 */
MFRC522::MFRC522(spi_device_handle_t spiHandle) : spi(spiHandle)
{
    spi = spiHandle;
} // End constructor
...
void MFRC522::PCD_ReadRegister(byte reg, byte count, byte *values, byte rxAlign)
{
    ...
    uint8_t reg1 = 0x80 | reg; // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
    spi_transaction_t t;
    memset(&t, 0, sizeof(t)); // Zero out the transaction
    t.length = 128;           // 8
    t.rxlength = 8 * count;
...
{
    // PCD_Version();
    byte ver = PCD_ReadRegister(VersionReg);
    ESP_LOGI(TAG, "Firmware version: 0x%02X", ver);  // Prints HEX
...
bool MFRC522::PCD_PerformSelfTest()
{
    ...
    ESP_LOGI(TAG, "Firmware version: 0x%02X", version);
    ...
    case 0x92: // Version 2.0
        ...
        break;
    default:
        ESP_LOGE(TAG, "Unknown firmware version: 0x%02X", version);
        return false;
    }
    ...
    for (byte i = 0; i < 64; i++)
    {
        ...
        {
            ESP_LOGE(TAG, "Self-test failed at byte %d: expected 0x%02X, got 0x%02X", i, reference[i], result[i]);
            return false;
        }
    }
...
// Raj Kumar
const char *MFRC522::GetStatusCodeName(byte code)
{
    switch (code)
    {
    case STATUS_OK:
...
// Raj Kumar
const char *MFRC522::PICC_GetTypeName(byte piccType)
{
    switch (piccType)
    {
...
void MFRC522::PICC_DumpToSerial(Uid *uid)
{
    ...
    for (byte i = 0; i < uid->size; i++)
    {
        if (uid->uidByte[i] < 0x10)
            ESP_LOGI(TAG, " 0%X", uid->uidByte[i]);
        else
            ESP_LOGI(TAG, " %X", uid->uidByte[i]);
    }

    // PICC type
    byte piccType = PICC_GetType(uid->sak);
    ESP_LOGI(TAG, "PICC type: %s\n", PICC_GetTypeName(piccType));
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid, MIFARE_Key *key, byte sector)
{
    ...
    // Each group has access bits [C1 C2 C3]. In this code C1 is MSB and C3 is LSB. The four CX bits are stored together in a nible cx and an inverted nible cx_.
    ...
    for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--)
    {
        ...
        for (byte index = 0; index < 16; index++)
        {
            if (buffer[index] < 0x10)
                ESP_LOGI(TAG, " 0%X", buffer[index]);
            else
                ESP_LOGI(TAG, " %X", buffer[index]);
            if ((index % 4) == 3)
            {
                ESP_LOGI(TAG, " ");
            }
        ...
        { // Not a sector trailer, a value block
            ...
            // ESP_LOGI(TAG, " Value=0x%X Adr=0x%X", value, buffer[12]);
            ESP_LOGI(TAG, " Value=0x%lx Adr=0x%X", value, buffer[12]);
        }
        ESP_LOGI(TAG, "");
    }
...
byte MFRC522::AuthenticateBlock(byte *uid, byte uidLen, uint32_t blockNumber, byte keyNumber, byte *keyData)
{
    // Create a Uid struct for authentication
    MFRC522::Uid nuid;
    for (byte i = 0; i < uidLen; i++)
    {
        nuid.uidByte[i] = uid[i];
    }
    nuid.size = uidLen;

    // Create a MIFARE_Key instance
    MFRC522::MIFARE_Key mifareKey;
    for (byte i = 0; i < MFRC522::MF_KEY_SIZE; i++)
    {
        mifareKey.keyByte[i] = keyData[i];
    }

    // Determine the authentication command based on key number
    byte command = (keyNumber == 0) ? MFRC522::PICC_CMD_MF_AUTH_KEY_A : MFRC522::PICC_CMD_MF_AUTH_KEY_B;

    // Authenticate the block
    byte result = PCD_Authenticate(command, blockNumber, &mifareKey, &nuid);
...
void MFRC522::ReadBlock(byte currentBlock)
{
    byte bufferSize = 18; // Buffer size must be at least 18 bytes
    byte data[18];        // Buffer to hold block data

    byte result = MIFARE_Read(currentBlock, data, &bufferSize);
    if (result == MFRC522::STATUS_OK)
    {
        ESP_LOGI(TAG, "Data from block %d: ", currentBlock);
...
*/
/**************************************************************************/
bool MFRC522::IsTrailerBlock(uint32_t uiBlock)
{
...
*/
/**************************************************************************/
bool MFRC522::IsFirstBlock(uint32_t uiBlock)
{
    // Test if we are in the small or big sectors
    if (uiBlock < 128)
        return ((uiBlock) % 4 == 0);
