...
static const char *TAG = "MifareUltralight";
...
NfcTag MifareUltralight::read()
{
    if (isUnformatted())
    {
        ...
        ESP_LOGW(TAG, "WARNING: Tag is not formatted.");
#endif
    ...
    for (uint8_t page = ULTRALIGHT_DATA_START_PAGE; page < ULTRALIGHT_MAX_PAGE; page+=(ULTRALIGHT_READ_SIZE/ULTRALIGHT_PAGE_SIZE))
    {
        ...
        bool success = nfc->MIFARE_Read(page, &buffer[index], &dataSize);

        if (success == MFRC522::STATUS_OK)
        {
        ...
        {
            ESP_LOGE(TAG, "Read failed %d", page);
            return NfcTag(nfc->uid.uidByte, nfc->uid.size, NfcTag::TYPE_2);
        }
...
bool MifareUltralight::isUnformatted()
{
    ...
    byte dataSize = ULTRALIGHT_READ_SIZE+2;
    byte data[dataSize];
    bool success = nfc->MIFARE_Read(page, data, &dataSize);

    if (success == MFRC522::STATUS_OK && dataSize >= 4)
    {
        return (data[0] == 0xFF && data[1] == 0xFF && data[2] == 0xFF && data[3] == 0xFF);
    }
    else
    {
        ESP_LOGE(TAG, "Error. Failed read page %d", page);
        return false;
    }
}
...
uint16_t MifareUltralight::readTagSize()
{
    ...
    bool success = nfc->MIFARE_Read(3, data, &dataSize);

    if (success == MFRC522::STATUS_OK && dataSize >= 2)
    {
        // See AN1303 - different rules for Mifare Family byte2 = (additional data + 48)/8
        tagCapacity = data[2] * 8;
#ifdef MIFARE_ULTRALIGHT_DEBUG
        ESP_LOGI(TAG, "Tag capacity %d bytes", tagCapacity);
#endif
...
#ifdef MIFARE_ULTRALIGHT_DEBUG
    ESP_LOGI(TAG, "messageLength %d", *messageLength);
    ESP_LOGI(TAG, "ndefStartIndex %d", *ndefStartIndex);
#endif
}
...
bool MifareUltralight::write(NdefMessage& m)
{
...
#ifdef MIFARE_ULTRALIGHT_DEBUG
        ESP_LOGW(TAG, "Encoded Message length exceeded tag Capacity %d", tagCapacity);
#endif
        return false;
    }
...
#ifdef MIFARE_ULTRALIGHT_DEBUG
    ESP_LOGI(TAG, "messageLength %d", messageLength);
    ESP_LOGI(TAG, "Tag Capacity %d", tagCapacity);
    ...
    while (position < bufferSize) { // bufferSize is always times pagesize so no "last chunk" check
        ...
        if (nfc->MIFARE_Write(page, writeBuffer, 16) != MFRC522::STATUS_OK)
            return false;
#ifdef MIFARE_ULTRALIGHT_DEBUG
        ESP_LOGI(TAG, "Wrote page %d - ", page);
