...
static const char *TAG = "NdefMessage";
...
NdefMessage::NdefMessage(const byte *data, const uint16_t numBytes)
{
    ...
    ESP_LOGI(TAG, "Decoding %d bytes", numBytes);
...
bool NdefMessage::addRecord(NdefRecord &record)
{
    ...
    {
#ifdef NDEF_USE_SERIAL
        ESP_LOGW(TAG, "WARNING: Too many records. Increase MAX_NDEF_RECORDS.");
#endif
        return false;
    }
}
...
{
    ESP_LOGI(TAG, "\nNDEF Message %d record", _recordCount);
    if(_recordCount == 1)
    {
     ESP_LOGI(TAG, ", ");
    }
    ...
    ESP_LOGI(TAG, "%d bytes", getEncodedSize());
