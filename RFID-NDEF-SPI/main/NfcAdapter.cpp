...
static const char *TAG = "NfcAdapter";
...
bool NfcAdapter::tagPresent()
{
    ...
    byte piccType = shield->PICC_GetType(shield->uid.sak);
...
bool NfcAdapter::format()
{
    ...
    {
#ifdef NDEF_USE_SERIAL
        ESP_LOGI(TAG, "No need for formatting a UL");
#endif
        return true;
    }
    else
...
bool NfcAdapter::clean()
{
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Cleaning Mifare Classic");
        #endif
        MifareClassic mifareClassic = MifareClassic(shield);
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Cleaning Mifare Ultralight");
        #endif
    ...
    {
#ifdef NDEF_USE_SERIAL
        ESP_LOGI(TAG, "No driver for card type %d", type);
#endif
        return false;
    }
}
...
NfcTag NfcAdapter::read()
{
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Reading Mifare Classic");
        #endif
        MifareClassic mifareClassic = MifareClassic(shield);
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Reading Mifare Ultralight");
        #endif
        MifareUltralight ultralight = MifareUltralight(shield);
    ...
    {
#ifdef NDEF_USE_SERIAL
        ESP_LOGI(TAG, "Cannot determine tag type");
#endif
...
bool NfcAdapter::write(NdefMessage& ndefMessage)
{
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Writing Mifare Classic");
        #endif
        MifareClassic mifareClassic = MifareClassic(shield);
    ...
    {
        #ifdef NDEF_DEBUG
        ESP_LOGI(TAG, "Writing Mifare Ultralight");
        #endif
